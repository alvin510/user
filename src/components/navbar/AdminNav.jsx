...
const Content = () => {
  ...
  return (
    <div className="min-h-screen p-8">
      ...
      {location.pathname === "/dashboard/ jadwal-show" && <JadwalAdd />}
      {location.pathname === "/dashboard/ jadwal-edit" && <JadwalShow />}
      {location.pathname === `/dashboard/jadwal-edit/${id}` && <JadwalEdit />}
      {location.pathname === "/dashboard/event-show" && <DashboardShow />}
      {location.pathname === "/dashboard/event-add" && <DashboardAdd />}
      {location.pathname === `/dashboard/event-edit/${id}` && <DashboardEdit />}
      {location.pathname === "/dashboard/profile" && <ProfileContent />}
      {location.pathname === `/dashboard/profile-edit/${id}` && <EditProfile />}
      {location.pathname === "/dashboard/profile-show" && <ShowProfile />}
      {location.pathname === "/dashboard/pengumuman-add" && <Pengumuman />}
      {location.pathname === "/dashboard/pengumuman-show" && <PengumumanShow />}
      {location.pathname === `/dashboard/pengumuman-edit/${id}` && (
        <PengumumanEdit />
      )}
      {location.pathname === "/dashboard/blog-add" && <Blog />}
      {location.pathname === "/dashboard/blog-show" && <BlogShow />}
      {location.pathname === `/dashboard/blog-edit/${id}` && <BlogEdit />}
      {location.pathname === `/dashboard/media-add` && <MediaAdd />}
      {location.pathname === `/dashboard/media-show` && <MediaShow />}
      {location.pathname === `/dashboard/media-edit/${id}` && <MediaEdit />}
    </div>
  );
};
